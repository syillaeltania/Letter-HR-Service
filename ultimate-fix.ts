...
async function buildTemplate(filename: string, templateName: string, title: string, dateString: string, signatoriesTable: string, pihakPertama: string, pihakKedua: string, pihakKeduaVar: string) {
  ...
  // Apply beautiful styling to Pasals so they look perfectly neat in Jodit!
  // 1. Style tables
  ...
  // 2. Style Paragraphs
  pasalContent = pasalContent.replace(/<p><strong>Pasal/gi, '<p style="font-weight: bold; margin-top: 15px; margin-bottom: 5px;"><strong>Pasal');
  // For all other paragraphs, make them justified and have margin
  pasalContent = pasalContent.replace(/<p(?! )/gi, '<p style="margin-bottom: 10px; text-align: justify; line-height: 1.6;"');

  // 3. Style Lists
  pasalContent = pasalContent.replace(/<ol>/gi, '<ol style="list-style-type: decimal; padding-left: 20px; margin-bottom: 15px; text-align: justify; line-height: 1.6;">');
  pasalContent = pasalContent.replace(/<ul>/gi, '<ul style="list-style-type: disc; padding-left: 20px; margin-bottom: 15px; text-align: justify; line-height: 1.6;">');
  pasalContent = pasalContent.replace(/<li>/gi, '<li style="margin-bottom: 5px;">');

  // Replace signature block at the end of pasals with the correct variables!
...
<table style="width: 100%; border-collapse: collapse; text-align: center; margin-top: 40px;">
  ...
  <tr>
    <td><strong>${pihakPertama}</strong><br><strong>{{signer_position}}</strong><br><strong>NIK. {{signer_nik}}</strong></td>
    <td></td>
    <td><strong>{{${pihakKeduaVar}}}</strong></td>
  </tr>
</table>
  `);
...
${pasalContent}
  ...
  console.log('✅ Ultimate Rebuilt', templateName);
}
...
async function run() {
  await buildTemplate('kontrak-kerja-karyawan.docx', 'Template Kontrak Kerja Karyawan', 'K O N T R A K   K E R J A', 'Pada hari ini, <strong><em>{{letter_day}}</em></strong> tanggal <strong><em>{{letter_date_text}}</em></strong> tahun <strong><em>{{letter_year_text}}</em></strong>, kami yang bertanda tangan di bawah ini :', sigKaryawan, 'Sriyanto', '{{employee_name}}', 'employee_name');
  await buildTemplate('kontrak-kerja-freelancer.docx', 'Template Kontrak Kerja Freelancer', 'K O N T R A K   K E R J A   F R E E L A N C E', 'Pada hari ini, <strong><em>{{letter_day_name}}</em></strong> tanggal <strong><em>{{letter_day_text}}</em></strong> bulan <strong><em>{{letter_month_text}}</em></strong> tahun <strong><em>{{letter_year_text}}</em></strong>, kami yang bertanda tangan di bawah ini :', sigFreelance, 'Sriyanto', '{{freelance_name}}', 'freelance_name');
  await buildTemplate('kontrak-kerja-magang.docx', 'Template Kontrak Kerja Magang', 'K O N T R A K   K E R J A   M A G A N G', 'Pada hari ini, <strong><em>{{letter_day_name}}</em></strong> tanggal <strong><em>{{letter_day_text}}</em></strong> bulan <strong><em>{{letter_month_text}}</em></strong> tahun <strong><em>{{letter_year_text}}</em></strong>, kami yang bertanda tangan di bawah ini :', sigMagang, 'Ryan Nurochman', '{{intern_name}}', 'intern_name');
}
