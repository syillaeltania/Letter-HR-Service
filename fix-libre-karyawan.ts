...
async function run() {
  const htmlFile = '/tmp/html/kontrak-kerja-karyawan.html';
  ...
  await prisma.letterTemplate.updateMany({
    where: { templateName: 'Template Kontrak Kerja Karyawan' },
    data: { templateContent: finalHtml }
  });
  console.log('✅ LibreOffice Updated Kontrak Kerja Karyawan');
}
