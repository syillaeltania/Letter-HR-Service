...
async function run() {
  ...
  for (const t of templates) {
    if (t.templateContent.includes('<!-- Body -->')) {
      const match = t.templateContent.match(/<!-- Body -->\s*<div style="margin-bottom: 20px; text-align: justify;">([\s\S]*?)<\/div>\s*<!-- Footer -->/);
      if (match) {
        let coreContent = match[1].trim();
        const newContent = htmlWrapper(coreContent);
        await prisma.letterTemplate.update({ where: { id: t.id }, data: { templateContent: newContent } });
